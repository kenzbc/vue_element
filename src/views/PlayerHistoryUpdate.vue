<template>
    <div class="player-history-update">
        <el-form ref="playerHistoryForm" :model="formData" label-position="right" label-width="150px" :rules="formRules">
...
<script>
	export default {
		...
		mounted () {
			if (this.$route.name === 'player-history-update') {
		...
		methods: {
			requestData (params) {
				const _this = this;
				this.ajax('match/player_history_op.php', params, function (response) {
			...
			requestUpdate () {
				...
				if (_this.$route.name === 'player-history-update') {
					_this.formData.act = 'edit';
				...
				this.ajax('match/player_history_op.php', this.formData, function (response) {
					if (response.data.code == 200) {
						_this.$message({
							message: response.data.message,
							type: 'success'
						});
						_this.$router.push({path: '/player-history',query:{playerID:_this.$route.query.playerID}})
					} else {
						_this.$message.error({
							message: response.data.message,
						});
			...
			submit () {
				this.$refs['playerHistoryForm'].validate((valid) => {
					if (valid) {
						this.requestUpdate()
					} else {
						return false;
