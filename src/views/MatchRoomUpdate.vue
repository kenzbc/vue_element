    ...
    <div class="room-update">
        ...
        <el-form ref="form" :model="formData" label-position="right" label-width="120px" :rules="formRules">
            ...
            <el-form-item label="房间描述：" prop="room_desc">
                <el-input v-model="formData.room_desc" type="text" placeholder="请输入房间描述（限30个字，一般用于填写奖励方案）" class="large"></el-input>
            </el-form-item>

            <el-form-item label="帖子ID：" prop="dynamicID">
                <el-input v-model="formData.dynamicID" type="text" placeholder="请输入帖子ID（用于范特西聊天室）" class="mini"></el-input>
            </el-form-item>
...
<script>
    export default {
        ...
        created () {
            if (this.$route.name === 'match-room-update') {
                this.action = 'update'
...
//                this.computeDistributionItems();
        ...
        methods: {
            requestRoomData (params) {
                const _this = this;
                this.ajax('edit_room.php',params, function (response) {
            ...
            requestUpdateRoom () {
                ...
                _this.form.code = -1;
                if (this.$route.name === 'match-room-create') {
                    this.ajax('create_room.php', 'post', this.formData, function (response) {
                        ...
                        if (response.data.code == 200) {
                            ...
                            _this.$router.push({path: '/match-room'})
                        } else {
                            _this.$message.error({
                                message: response.data.message,
                            });
