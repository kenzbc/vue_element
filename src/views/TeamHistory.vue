<template>
    <div class="team-history-list">
        <el-form :inline="true" :model="queryParams">
            ...
            <el-form-item  style="float:right;margin-right: 0">
                <router-link :to="{ path:'/team-history/create', query: { teamID:  this.$route.query.teamID} }">
        ...
        <el-table :data="tableData" border stripe  v-loading="loading">
            ...
            <el-table-column label="操作" class="center" align="center" width="240">
                <template scope="scope">
                    <router-link :to="{ path:'/team-history/'+ scope.row.id, query: { teamID:  $route.query.teamID} }" tag="el-button" class="el-button--small el-button--primary">编辑</router-link>
    ...
    export default {
        ...
        methods: {
            ...
            requestList (data) {
	            ...
	            this.ajax('match/team_history.php', this.queryParams, function (response) {
                ...
                switch (type) {
                    case 'del':
                        this.$confirm('确定删除吗？', '提示', {
                            ...
                            _this.ajax('match/team_history_op.php', {id: row.id,act:'del'}, function (response) {
                                if (response.data.code == 200) {
                                    _this.$message({
                                        type: 'success',
                                        message: response.data.message
	                ...
	                case 'refresh':
		                this.$confirm('确定采集吗？', '提示', {
			                ...
			                _this.ajax('match/team_history_op.php', {teamID: row.teamID,act:'refresh'}, function (response) {
				                if (response.data.code == 200) {
					                _this.$message({
						                type: 'success',
						                message: response.data.message
